...
# view_range <- get_view_range(tx_data = tx_data, symbol = symbol, 
                             # keytype = "SYMBOL")
...
# Get the exon list
exon_data <- get_tx_annotation(range = view_range, 
...
cvg_gr <- make_coverage_tracks(fi[idx, "File.bw"], view_range, 
...
```{r Basic Track}
track_info <- plot_track_view(cvg = cvg_gr, target_range = view_range, 
...
ptv <- make_track_function(cvg_files = cvg_files,
                           genome = genome, tx_data = tx_data,
