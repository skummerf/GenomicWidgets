...
All data for this presentation is from: Mohammed, H. *et al.* Progesterone receptor modulates ERα action in breast cancer. *Nature*. **523**, 313-317 (2015)
...
rna_hm <- genomics_heatmap(v$E[sig,], 
                           ...
                           colorbar_grid =setup_colorbar_grid(x_spacing = 0.2, 
                                                              nrow = 3),
                           colorscale = 
                             diverging_colorscale(c(scales::muted("blue"),
                                                    "white", 
                                                    scales::muted("red"))),
                           font = list(size = 16)) 
...
```{r}
tss <- transcriptsBy(TxDb.Hsapiens.BioMart.igis, by = "gene") %>% unlist()
tss$entrez <- stringr::str_extract(names(tss),"(?<=:)(.*)")
tss$symbol <- annotate::getSYMBOL(tss$entrez, data='org.Hs.eg')
names(tss) = NULL
unique_tss <- unique(tss) #Remove same positions

peaks <- lapply( fi$File.macs[unlist(T47D)], readMacsPeaks, fc=5, minTags=10, fdr=0.0001)
...
tss_in_sig <- unique_tss[which(unique_tss$symbol %in% fData(T47D_eset)$symbol[sig])]
...
single_coverage_heatmap(tss_cvg_mats[[2]], row_order = "signal", 
                        y = tss_in_sig$symbol, font = list(size = 20))
```


```{r}
merged_peaks2 <- lapply(T47D,
                       function(x){
...
single_coverage_heatmap(er_progesterone_peaks_cvg_mats[[2]], row_order = "signal",  k = 3, font = list(size = 20)) 

names(er_progesterone_peaks_cvg_mats)[c(1,2,4,5)] <- c("ER Antibody<br>Full Media",
                                                       "ER Antibody<br>P4 Treatment",
                                                       "PR Antibody<br>Full Media",
                                                       "PR Antibody<br>P4 Treatment") 

mchm = multi_coverage_heatmap(er_progesterone_peaks_cvg_mats[1:2], 
                       row_order = "kmeans", k = 2, cluster_by = "first", font = list(size = 16), name = "ER<br>Coverage", signal_name = "Er Avg.") 

mchm %>%
  add_multi_coverage_heatmap(er_progesterone_peaks_cvg_mats[4:5],font = list(size = 16), groups = mchm$data$xaxis5$yaxis$groups , name = "PR<br>Coverage", signal_name = "Pr Avg.")
...
rna_chip_collapsed_hm <- rna_hm %>% add_genomics_heatmap(er_promoter_counts_collapsed,
  ...
  add_genomics_heatmap(p300_promoter_counts_collapsed,
                      ...
                      show_col_groups_colorbar = FALSE,
                              name = "p300 ChIP",
                              size = 0.3,
                              x_title = "p300", scale = "none")
```

```{r}
rp_scores <- sapply(merged_peaks2, compute_regulatory_potential, tss_ranges[sig])

rna_rp_hm <- rna_hm %>% add_genomics_heatmap(rp_scores[,1:3],
                              col_groups = c("Full Media", "P4",
                                             "R5020"),
                              col_groups_name = "Hormone<br>Treatment",
                              col_groups_palette = "Paired",
                              ...
                              size = 0.3,
                              x_title = "ER", scale = "none") %>%
  add_genomics_heatmap(rp_scores[,4:6],
                              col_groups = c("Full Media", "P4",
                                             "R5020"),
                              col_groups_palette = "Paired",
                       colorscale = continuous_colorscale("Reds"),
                      show_col_groups_colorbar = FALSE,
                              ...
                              size = 0.3,
                              x_title = "PR", scale = "none")%>%
  add_genomics_heatmap(rp_scores[,7:9],
                              col_groups = c("Full Media", "P4",
                                             "R5020"),
                              col_groups_palette = "Paired",
                       colorscale = continuous_colorscale("Reds"),
...
```{r}
##tmp function for now till fixed issue
update_annotation_fonts <- function(p, anno_size = 16, other_size = 16){
  for (i in seq_along(p$layout$annotations)){
    p$layout$annotations[[i]]$font$size = anno_size
  }
  p$layout$font$size = other_size
  return(p)
}

rna_chip_collapsed_hm2 <- update_annotation_fonts(rna_chip_collapsed_hm, 32, 24)

rna_chip_diff_hm2 <- update_annotation_fonts(rna_chip_diff_hm, 32, 24)

rna_rp_hm2 <- update_annotation_fonts(rna_rp_hm, 32, 24)
...
```{r}
plot_browserly_tracks(symbol_range, tx_data, range_cvg)
```
...
plot_browserly_tracks(symbol_range, tx_data, subset_coverage(range_cvg,cvg_subset))
```
...
link_function <- heatmap_click(rna_chip_diff_hm, tss_ranges[sig])

bw_files <- fi$File.bw[unlist(T47D)]
names(bw_files) <- sample_names
track_function <- make_browserly_function(bw_files, tx_data)

heatmap_to_browserly_shiny(rna_chip_diff_hm, track_function, link_function)
