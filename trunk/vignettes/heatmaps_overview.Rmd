...
```{r}
new_single_coverage_heatmap(cvg_mats[[1]], row_order = "none") %>% plot_iHeatmap()
```
...
```{r}
new_single_coverage_heatmap(cvg_mats[[1]], row_order = "hclust") %>% plot_iHeatmap()
```
...
new_single_coverage_heatmap(cvg_mats[[1]], row_order = "hclust",  clust_dist = cor_dist) %>% plot_iHeatmap()
```
...
```{r}
new_single_coverage_heatmap(cvg_mats[[1]], row_order = "hclust", k = 3) %>% plot_iHeatmap()
```
...
```{r}
new_single_coverage_heatmap(cvg_mats[[1]], row_order = "kmeans", k = 3) %>% plot_iHeatmap()
```
...
```{r}
new_single_coverage_heatmap(cvg_mats[[1]], row_order = "kmeans", k = 3, summary = FALSE) %>% plot_iHeatmap()
```
...
new_single_coverage_heatmap(peak_cvg_mats1, row_order = "signal", signal = rowSums(peak_cvg_mats1)) %>% 
  add_dist_to_tss(peaks[[mRNF2[1]]][1:1000],tss) %>% plot_iHeatmap()
...
```{r multi_coverage}
multi_coverage_heatmap(cvg_mats[1:3], row_order = "kmeans", k = 3)  %>% plot_iHeatmap()
```
...
```{r multi_coverage2}
multi_coverage_heatmap(cvg_mats[1:3], row_order = "kmeans", k = 3, cluster_by = "all")  %>% plot_iHeatmap()
```
...
p1 %>% 
  add_simple_heatmap(promoter_counts, col_order = "hclust", k_col = 4, scale_method = "normalize", name = "ChIP") %>%
  plot_iHeatmap()  %>% 
