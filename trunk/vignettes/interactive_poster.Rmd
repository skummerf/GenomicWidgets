...
# Interactive RNA Heatmap
 
Heatmap showing relative RNA expression across samples treated with E2 (estrogen), E2 plus P4 (progesterone), or E2 plus R5020 (synthetic P4). 
...
# Add ChIP-seq data to RNA heatmap

We can add the differential ChIP peaks for P4 and R5020 treatment for several factors (ER, PR, and p300) next to the RNA heatmap.  Each row of the ChIP heatmaps represents the differential ChIP signal in a 10000 bp window around the transcription start site of the matching gene in the RNA-seq heatmap.
...
# Individual gene views

We can also look at the ChIP-seq signal around a particular gene of interest.  If looking at many ChIP-seq samples, we can plot the coverage as a heatmap.

```{r}
plot_browserly_tracks(symbol_range, tx_data, range_cvg, binsize = 100)
```

If we want to focus in on just a couple ChIP-seq samples, we can make a line plot instead. 
