...
for reference
``` {r Load Data}
info_dir <- "/gne/research/workspace/schepa/Carroll_Data/CARROLL_PR/data/"
...
exon_data <- get_tx_annotation( range = symbol_range, 
                               tx_data = tx_data, no_introns=TRUE)
cvg <- make_coverage_tracks(fi[idx, "File.bw"], symbol_range, 
                                      sample_names = shortened_names, 
                            scaling_factors = sf, binsize = 500)
...
```{r Standard plot}
track_info <- plot_track_view(cvg = cvg, target_range = symbol_range, 
...
sf <- get_scaling_factor(fi[idx,])/1000000
sf <- sf[2,]
more_cvg <- make_coverage_tracks(fi[idx, "File.bw"], symbol_range, 
                                      sample_names = shortened_names, 
                            scaling_factors = sf, binsize = 500)
track_info <- plot_track_view(more_cvg, symbol_range, exon_data, genome)
```
...
``` {r Plot with TSS}
tss_db <- tssFromTxdb(TxDb.Hsapiens.BioMart.igis)
track_info <- plot_track_view(more_cvg, symbol_range, exon_data, genome, 
                                       tss_gr = tss_db)
```
...
track_info <- plot_track_view(more_cvg, symbol_range, exon_data, genome, 
                              tss_gr = tss_db, snp_gr = snps, snptrack_kwargs = list(legend=TRUE))
```
...
colors <- c('red', 'black')
# Then we just supply it as a list of kwargs
track_info <- plot_track_view(cvg, symbol_range, exon_data, genome, 
