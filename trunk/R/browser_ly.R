...
plot_multiple_genes <- function(genes,
  ...
  cvg_tracks <- make_subplots(grl = cvg_list, type = type, fill, mode, ...)
  ...
  heights <- rep(c((0.65/length(genes)), (0.35/length(genes))), length(genes))
  sp <- subplot(plots, shareX = TRUE, nrows=length(plots), heights = heights)
  ...
  sp <- modify_y(sp, sp_info, grt_ax, type, sync_y = sync_y, native_title = TRUE)
  sp$x$layout$xaxis$range <- display_range
  sp$x$layout$xaxis$zeroline <- FALSE
  sp$x$layout$margin <- list(t=0)
  return(sp)
}
...
browserly_cvg_track <- function(cvg,
  ...
  if(is.null(colorpalette)) {
    colorpalette <- rev(RColorBrewer::brewer.pal(8, "Dark2"))
  }
