---
title: "Breast Cancer multi-omics analysis"
runtime: shiny
author: "Alicia Schep, Justin Finkle, Sarah Kummerfeld"
date: "`r Sys.Date()`"
...
tss_ranges <- with(fData(rpkmEset), GenomicRanges::GRanges(chr,
  IRanges::IRanges(start = ifelse(strand == "-", end, start), width = 1),
                                               strand = strand,
                                               symbol = symbol,
                                               entrez = entrez))
...
se <- SummarizedExperiment(assays=SimpleList(exprs=v$E), 
                           rowData=fData(T47D_eset),
                           colData=pData(T47D_eset))


rna_hm <- iheatmap(se[sig], "exprs",
    x = pData(T47D_eset)$title, 
    y = fData(T47D_eset)$symbol[sig],
    col_annotation = data.frame(group=treat_groups))
...
```{r Match transcripts with symbol}
tr <- transcriptsBy(TxDb.Hsapiens.BioMart.igis, by = "gene") %>% unlist()
tr$entrez <- stringr::str_extract(names(tr),"(?<=:)(.*)")
tr$symbol <- annotate::getSYMBOL(tr$entrez, data='org.Hs.eg')
names(tr) = NULL
...
plot_browserly_tracks(range = symbol_range, tx_data = tx_data, cvg_list = range_cvg[cvg_subset], binsize=100)
```
