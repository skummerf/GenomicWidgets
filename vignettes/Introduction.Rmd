...
knitr::opts_chunk$set(echo = TRUE, figure.width = 6, figure.height = 5)
```
...
```{r}
track_plotter(resize(ctcf.peaks[2], width = 10000, fix = "center"))
```
...
```{r}
track_plotter2 <- make_track_plotter(samp.info$fileName[1:4], 
                                    annotation = 
                                      TxDb.Hsapiens.UCSC.hg19.knownGene, 
                                    track_names = samp.info$sampleName[1:4], 
                                    share_y = TRUE,
                                    groups = c("CTCF & cohesin","P300","Suz12","CTCF & cohesin"))
```

```{r}
track_plotter2(resize(ctcf.peaks[2], width = 10000, fix = "center"))
```
...
```{r}
track_plotter2(resize(ctcf.peaks[1:3], width = 10000, fix = "center"))
```

### Locus names

```{r}
track_plotter(resize(ctcf.peaks[1:2], width = 10000, fix = "center"),
              name = c("Peak 1","Peak 2"))
```
...
The properties of the tracks or summary plots can be altered by adding track_args or summary_args arguments.

```{r}
combined_plotter(rownames(rpkm_chr21)[1:3], 
                 track_args = list(name = rowData(rpkm_chr21)$SYMBOL[1:3]),
