...
If the same files and annotation data will be used multiple times, but the view range will change, it is often more convenient to have a wrapper function around `plot_single_locus`. In this example we abstract the function call so that only the target range is provided to the new function. This simplification is particularly useful when using browserly in a Shiny application.
```{r Function abstraction}
trackview_function <- make_browserly_function(cvg_files = fi[idx, "File.bw"],
                                              tx_data = tx_data, 
                                              sample_names = shortened_names, 
                                              cvg_scaling = sf,
                                              stacking = 'squish',
...
Now we can plot new regions very easily.
```{r Function plot}
# Let's switch genes and plot NRIP1
new_view <- extend_grange(get_view_range(chr = 21, start =  14961235, end = 15065936), 20000)
trackview_function(new_view)
```
